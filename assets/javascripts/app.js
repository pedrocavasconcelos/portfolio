...
(function($) {
  Tc.Module.MasonryGallery = Tc.Module.extend({
    ...
    onBinding: function() {
      ...
      // Thanks to: https://stackoverflow.com/questions/38765659/how-can-i-use-masonry-multiple-times-per-page-with-the-same-class-name
      var galleries = document.getElementsByClassName('gallery');
      // console.log(galleries)
      var n = galleries.length;

      for(var i = 0; i < n; i++){
        (function ()  {

      
        var msnry = new Masonry(galleries[i], { itemSelector: 'li', gutter: gutter, isInitLayout: false, columnWidth: ".modMasonryGallery li:not(.wide)" });
        // console.log(galleries[i])
        window.msnry = msnry;

        // Call imagesLoaded for each Masonry instance
        $(galleries[i]).imagesLoaded(function () {
          msnry.layout();
        });
